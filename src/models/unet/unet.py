...
class UNetModel(nn.Module):
    ...
    def condition_embedding(self, conditions: torch.Tensor, max_period: int = 10000):
        """
        ## Create sinusoidal condition embeddings

        :param time_steps: are the conditions of shape `[batch_size]`
        :param max_period: controls the minimum frequency of the embeddings.
        """
        # $\frac{c}{2}$; half the channels are sin and the other half is cos,
        half = self.channels // 2
        ...
        frequencies = torch.exp(
            ...
            * torch.arange(start=0, end=half, dtype=torch.float32)
            / half
        ).to(device=conditions.device)
        # $\frac{t}{10000^{\frac{2i}{c}}}$
        args = conditions[:, None].float() * frequencies[None]
        # $\cos\Bigg(\frac{t}{10000^{\frac{2i}{c}}}\Bigg)$ and $\sin\Bigg(\frac{t}{10000^{\frac{2i}{c}}}\Bigg)$
        return torch.cat([torch.cos(args), torch.sin(args)], dim=-1)
    ...
    def forward(self, x: torch.Tensor, time_steps: torch.Tensor, cond: torch.Tensor):
        ...
        cond_emb = self.condition_embedding(cond)
        cond_emb = self.cond_embed(cond_emb)
        cond_emb = cond_emb.reshape(cond_emb.shape[0], 1, -1)
